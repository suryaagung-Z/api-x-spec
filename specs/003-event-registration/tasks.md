...
- [ ] T029 [P] Run `black --check` and `ruff check` on all new/modified modules (`src/api/schemas/registrations.py`, `src/api/routers/registrations.py`, `src/application/registration_service.py`, `src/infrastructure/repositories/registration_repository.py`, `src/domain/models.py`, `src/domain/exceptions.py`, `src/infrastructure/db/models.py`); fix any formatting or lint violations before merge
